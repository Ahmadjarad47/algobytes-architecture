...
public sealed class ConfirmUserEmailCommandHandler(
    UserManager<ApplicationUser> userManager,
    IApplicationDbContext db,
    IAccessPolicyEvaluator accessPolicyEvaluator)
    : IRequestHandler<ConfirmUserEmailCommand, Unit>
{
    public async Task<Unit> Handle(ConfirmUserEmailCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!await scoped.AnyAsync(cancellationToken))
        {
            ...
            {
                new ValidationFailure(nameof(ConfirmUserEmailCommand.UserId), "User was not found."),
            });
        }
